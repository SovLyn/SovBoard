...
fn cleanup_orphan_images(entries: Vec<ImageEntry>) -> Result<CleanupResult, String> {
    ...
    if let Ok(dir) = fs::read_dir(&scan_dir) {
        for de in dir {
            ...
            let fp = de.path();
            if !fp.is_file() || !is_image_file(&fp) {
                continue;
            }
            let canon = fp
                .canonicalize()
            ...
            if !known.contains(&canon) {
                match fs::remove_file(&fp) {
                    Ok(()) => {
                        removed.push(fp.to_string_lossy().to_string());
                    }
                    Err(e) => {
                        errors.push(format!("删除 {}: {}", fp.display(), e));
                    }
                }
            }
        }
...
fn show_quick_selector(app: tauri::AppHandle, entries_json: String) -> Result<(), String> {
    ...
    *st.entries_json.lock().map_err(|e| e.to_string())? = entries_json;
    ...
    w.emit("quick-selector:entries-updated", json)
...
fn request_file(
    ...
    match &p2p.download_tx {
        Some(tx) => tx
            .send(p2p::DownloadRequest { hash, save_dir })
            .map_err(|e| format!("{}", e)),
...
async fn cancel_download(
    ...
    match &p2p.cmd_tx {
        Some(tx) => tx
            .send(p2p::Command::CancelDownload { hash })
            .map_err(|e| format!("{}", e)),
        None => Err("P2P 未初始化".into()),
    }
}
...
async fn get_peer_list(
    ...
    Ok(s.peers
        ...
        .map(|(id, a)| PeerInfo {
            ...
            addresses: a.iter().map(|x| x.to_string()).collect(),
        })
        .collect())
}
